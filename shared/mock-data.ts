...
import { runCodingEngine, classifyAutomationPhase } from './coding-engine';
import { generateCdiNudges } from './cdi-rules';
...
// never drift from the engine's actual behavior.
interface NotePlan {
  encounter_id: string;
  note: string;
...
// --- SEED CDI NUDGES: also generated live from the same notes/engine pass ---
export const MOCK_NUDGES: Nudge[] = NOTE_PLAN.flatMap((plan) =>
  generateCdiNudges(plan.note, plan.encounter_id, { age: plan.age, encounterType: plan.encounter_type })
).map((nudge, index) => ({
  ...nudge,
  // Vary status across the seed set so the console demo shows every filter state.
...
export const MOCK_ANALYTICS: Analytics[] = MOCK_CODING_JOBS.map((job, index) => ({
  id: `a${index + 1}`,
  job_id: job.id,
